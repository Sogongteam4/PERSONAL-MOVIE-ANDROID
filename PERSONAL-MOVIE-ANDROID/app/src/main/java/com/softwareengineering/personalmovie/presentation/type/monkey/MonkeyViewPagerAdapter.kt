...
class MonkeyViewPagerAdapter(private val context:Context, private val movieList:List<Movie>):
    RecyclerView.Adapter<MonkeyViewPagerAdapter.MonkeyPagerViewHolder>() {
    inner class MonkeyPagerViewHolder(private val binding: ItemTypeBinding) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MonkeyPagerViewHolder {
        val binding =
            ItemTypeBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return MonkeyPagerViewHolder(binding)
    }
    override fun onBindViewHolder(holder: MonkeyPagerViewHolder, position: Int) {
