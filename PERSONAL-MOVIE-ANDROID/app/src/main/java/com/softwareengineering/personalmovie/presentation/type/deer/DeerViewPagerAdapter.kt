...
class DeerViewPagerAdapter(private val context:Context, private val movieList:List<Movie>):
    RecyclerView.Adapter<DeerViewPagerAdapter.DeerPagerViewHolder>() {
    inner class DeerPagerViewHolder(private val binding: ItemTypeBinding) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): DeerPagerViewHolder {
        val binding =
            ItemTypeBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return DeerPagerViewHolder(binding)
    }
    override fun onBindViewHolder(holder: DeerPagerViewHolder, position: Int) {
